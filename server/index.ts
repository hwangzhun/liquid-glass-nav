...
import { findSiteIcons } from "../shared/siteIcons.js";
import { validatePublicIconTarget } from "../shared/nodeSiteIconSecurity.js";
...
async function startServer() {
  ...
  app.get("/api/auth", async (req, res) => {
    ...
    res
      ...
      .json({ authenticated, configured: true });
  });
  ...
  app.post("/api/auth", async (req, res) => {
    if (!navPassword) {
      res
        .status(503)
        .json({ error: "NAV_PASSWORD 尚未配置。", configured: false });
      return;
    }
    const submitted =
  ...
  app.post("/api/analyze-site", async (req, res) => {
    try {
      if (
        !navPassword ||
        !(await hasValidSession(req.headers.cookie, navPassword))
      ) {
  ...
  app.post("/api/site-icons", async (req, res) => {
    try {
      if (
        !navPassword ||
        !(await hasValidSession(req.headers.cookie, navPassword))
      ...
      if (!url) {
        res.status(400).json({ error: "请填写网站地址。" });
        return;
      }
      const allowProxyFakeIps =
        process.env.ALLOW_PROXY_FAKE_IPS?.toLowerCase() === "true";
      res.json({
        icons: await findSiteIcons(url, target =>
          validatePublicIconTarget(target, { allowProxyFakeIps })
        ),
      });
    } catch (error) {
      res.status(400).json({
